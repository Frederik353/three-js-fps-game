...
// m416
import m416 from "./assets/guns/m416/m416.glb";
// grapling gun
import pistol from "./assets/guns/graplingGun/graplingGun.glb";
import GraplingGun from "./entities/Player/GraplingGun";
...
class FPSGameApp {
    ...
    SetupPhysics() {
        ...
        // used to be btDiscreteDynamicsWorld
    ...
    setupDebuger() {
        // oppdateres i entity rekkefølge viktig at kamera oppdateres sist slik at alt er i de oppdaterte posisjonene når bilde rendres
        if (window.location.href.indexOf("#debug") > -1){
            this.entityManager.entities.forEach((v, i) => {
                if (v.name.includes("Mutant")) {
                    v.AddComponent(new DirectionDebug(this.scene));
                    v.components["DirectionDebug"].Initialize(); // initialiseres egentlig ved addEntity men denne funksjonen kjøres i etterkant
                }
            })
            ...
            debugEntity.SetName("debug");
            debugEntity.AddComponent(new DebugPanel(this));
            debugEntity.AddComponent(new DebugCamera(this.scene, this.camera));
    ...
    async LoadAssets() {
        ...
        // m416
        promises.push(this.AddAsset(m416, gltfLoader, "m416"));
        // grapling gun
        promises.push(this.AddAsset(pistol, gltfLoader, "graplingGun"));
        ...
        this.assets["m416"].scene.animations = this.assets["m416"].animations;
        this.assets["graplingGun"].scene.animations = this.assets["graplingGun"].animations;
    ...
    EntitySetup() {
       ...
       playerEntity.AddComponent(new PlayerPhysics(this.physicsWorld, Ammo));
        playerEntity.AddComponent(
            new PlayerControls(this.camera, this.scene)
        );
        playerEntity.AddComponent(
            new Weapon(
                this.camera,
                this.assets["m416"].scene,
        ...
        // playerEntity.AddComponent(new Weapon(this.camera, this.assets['m416'].scene, this.assets['muzzleFlash'], this.physicsWorld, this.assets['ak47Shot'], this.listener ));
        ...
        // const ammoLocations = []
        const ammoLocations = [ [14.37, 0.0, 10.45], [32.77, 0.0, 33.84], ];
        ...
        let player = playerEntity.GetComponent("PlayerControls")
        player.guns[player.currentGun].ToggleEquipped();
    ...
    WindowResizeHanlder = () => {
        ...
        // const innerWidth = this.canvas.width;
        // const innerHeight = this.canvas.height;
