...
export default class PlayerControls extends Component {
    ...
    Initialize() {
        ...
        Input.AddMouseWheelListener(this.SwitchWeapon);
    ...
    SwitchWeapon = (event) => {
        ...
        let prevGun = this.guns[this.currentGun];
        this.currentGun = (this.currentGun + 1) % this.guns.length;
        let currentGun = this.guns[this.currentGun];
        console.log(this.guns[this.currentGun].name);
        if (prevGun.shoot) {
            currentGun.shoot = prevGun.shoot;
            prevGun.shoot = false;
        }
        prevGun.ToggleEquipped();
        currentGun.ToggleEquipped();
    };

    ShootCurrentGun(e) {
        if (this.guns[this.currentGun].shoot) {
            if (e.button != 0) return;
        } else {
            if (
                !Input.isLocked ||
                e.button != 0 ||
                this.guns[this.currentGun].reloading
            ) {
                return;
            }
        }
        this.guns[this.currentGun].ToggleShoot();
    }
    ...
    Update(t) {
        ...
        if (Input.GetKeyDown("Space")) {
            if (
                this.physicsComponent.jumps < this.physicsComponent.maxJumps &&
                this.physicsComponent.canJump
            ) {
                velocity.setY(this.jumpVelocity);
                this.physicsComponent.canJump = false;
                this.physicsComponent.jumps++;
            }
        } else {
            this.physicsComponent.canJump = true;
        }
