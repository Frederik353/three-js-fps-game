...
export default class GraplingGunFSM extends FiniteStateMachine{
    constructor(proxy){
        super();
        this.proxy = proxy;
        this.Init();
    ...
    Init(){
        ...
        this.AddState('equip', new EquipState(this));
    }
}
...
class ShootState extends State{
    ...
    Enter(prevState){
        ...
        // action.timeScale =   action._clip.duration / this.parent.proxy.fireRate;
        action.play();
    }
...
class EquipState extends State{
    constructor(parent){
        super(parent);
    ...
    get Name(){ return 'equip'; }
    get Animation(){ return this.parent.proxy.animations['equip']; }

    AnimationFinished = e => {
        if(e.action != this.Animation.action){
            return;
        }
        this.parent.proxy.EquipDone();
        this.parent.SetState('idle');
    }
